...
This package provides a set of quick solution functions for streamflow data tidying (add_ and fill_ functions), screening (screen_ functions), statistical analyses (calc_ and compute_ functions), and visualization (plot_ functions), amongst others.
...
This package allows for multiple stations (or other groupings such as time-periods of a flow record (ex. pre/post an event)) to be analyzed in many of the functions provided station identifiers are provided using the `groups` column argument (defaults to STATION_NUMBER). If grouping column doesn't exist or is improperly named, then all values listed in the `values` column will be summarized. Note that the plotting functions do not use the `groups` arguments (as just one plot is typically produced), so just one station/group should be provided.
