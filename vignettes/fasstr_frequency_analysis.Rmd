...
This vignette guide contains a guide on the various volume frequency analysis functions found in fasstr to plot frequency data and determine frequency quantile statistics. The methods for these analysis follow those used in the US Army Corps of Engineers HEC-SSP Statistical Software Package. See this [website](http://www.hec.usace.army.mil/software/hec-ssp/) for more information on the software and links to their User Manual for more in-depth information.


## Introduction

Computing a frequency analysis with fasstr allows for options and customization of both the inputs and outputs. These functions plot probabilties of flow data using chosen plotting options and calcualtes frequency quantiles (ex. 7Q10) based on fitting data to distributions using selected fitting methods. There are four functions within fasstr that conduct frequency analyses, each either their own purpose:
...
3) **compute_HYDAT_peak_frequencies()** - cconduct an annual frequency analysis from instantaneous peak data (minimum or maximum) for stations from HYDAT.
...
The probability plotting postions (A and B constants) are selected using the `prob_plot_position` argument, listing `'weibull'` where A and B are 0, `'median'` where A and B are 0.3, or `'hazen'` where A and B are 0.5. The selected plotting position does not have an effect on the final computed curve. 
