...
DOES IT NEED TO BE INSTALLED BEFORE READING VIGNETTES????
...
Tibble data frames, same as data frames.
Writing functions can save as csv, xls, xlsx files `write_results()` function (see section XXXX for more information).

ggplot2 objects, all plots within a list, so provided `write_plots()` function to directly save the list of plots within a directory or single PDF document. (see section XXX for more information)
so they can be modified using ggplot2 functions
...
To ensure that analyses do not skip over dates, this function looks for gaps in dates, adds the dates and fills in the flow values with NA. It does not do any gap filling, it just gives those values NA. It also fills dates to create complete start and end years. For example, if data starts in April, all flow values starting in the previous January will be filled with NA. The timing of the year depends on the `water_year` and `water_year_start` arguments. When `water_year = FALSE` (default), it will fill to complete calendar years (Jan-Dec). If `water_year = TRUE` then it will fill to compelte water years, defaulting to Oct-Sep, or other year periods with a different starting month with `water_year_start`.

Ideal to fill missing dates before using other `add_*` functions so dates added are not missing the other new values.
...
Default 'Year', 'Month' (numeric), 'MonthName' (month abbreviation), and 'DayofYear.' If `water_year = TRUE` then it will add 'WaterYear' and 'WaterDayofYear' columns in addition. Water year will start depending on the `water_year_start` argument, that defaults to "9", or October.
...
To added some seasons columns to the data...
...
Adds columns of rollings means.
`roll_days` indicate the rolling mean days, single or mulitple days
`roll_align` indicates the alignment of the rolling mean compared to the date. Options are `left`, `right`, and `center`, left meaning.....

 if using right or centre and data begins at the start of a desired period, there may be 
 
 show example, the 3 days with "left" show the date is on the "left" or first of all 3 flow values. The 5 days with "center" show the date is in the centre of all 5 flow values (if an even number, then half the flows are after the date, and the other half are the date and the previous flow values). The 7 days with "right" show the date is on the "right" or last of all 7 flow values.
