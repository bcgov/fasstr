...
To install the `fasstr` you will first need to install `remotes`, if not already done so, and then `fasstr` using the following code. It may take a few moments as there are several dependecy packages will also be installed, including [tidyhydat](https://CRAN.R-project.org/package=tidyhydayt) for downloading Water Survey of Canada hydrometric data, [zyp](https://CRAN.R-project.org/package=zyp) for trending, [ggplot2](https://cran.r-project.org/web/packages/ggplot2/index.html) for creating plots, and [dplyr](https://CRAN.R-project.org/package=dplyr) and [tidyr](https://CRAN.R-project.org/package=tidyr) for various data wrangling and summarizing functions, amongst others. 
