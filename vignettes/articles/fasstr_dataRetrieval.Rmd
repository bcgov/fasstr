...
There are several functions that are used to prepare your flow data set for your own analysis. These functions begin with `add_` or `fill_` and add columns or rows, respectively, to your flow data frame. See the documentation for more info on the functions. The following example shows how to apply the tidying functions to USGS data in a cotinuous pipeline and using water years. Note how the basin area correction is applied in the two `_cumulative_` functions and columns are renamed to match the appropriate units where necessary.
...
Screening, calc_ and plot. Some require renaming of names
