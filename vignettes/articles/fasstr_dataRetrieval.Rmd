...
# Plot the winter-spring (Jan-July) center of volume dates
...
# Calculate daily summary statistics, including 25 and 27 percentiles
calc_daily_stats(usgs_data,
                 percentiles = c(25,75),
                 water_year_start = 10)

# Plot an annual hydrograph (daily summary statistics) using only data 
# from complete years and add 2020 daily data
plot_daily_stats(usgs_data,
                 complete_years = TRUE,
                 add_year = 2020)
...
plot_daily_cumulative_stats(usgs_data,
                            add_year = 2020,
                            use_yield = TRUE,
                            basin_area = usgs_basin_area * 2323.2)[[1]]+
...
The `compute_annual_trends()` function calculates prewhitened non-parametric Mann-Kendall annual trends on streamflow data using the [`zyp`](https://CRAN.R-project.org/package=zyp) package. The function calculates various annual metrics using the `calc_all_annual_stats()` function and then calculates and plots the trending data. The `zhang` prewhitening method is recommended for hydrologic applications over `yuepilon`. See the [`zyp`](https://CRAN.R-project.org/package=zyp) package and the [trending vignette](https://bcgov.github.io/fasstr/articles/fasstr_trending_analysis.html) for more information.
