...
NWIS data column names can easily be used with fasstr functions, with setting `values = X_00060_00003` or `values = 'X_00060_00003'` to identify the flow values column. If there are multiple stations in a data frame, the stations can be grouped separately by identifying them using `groups = site_no` or `groups = 'site_no'`. Date column names are the same in both NWIS and HYDAT data.
...
# from complete years and add 2020 daily data for comparison
...
- US introduction
- couple more multiple station examples