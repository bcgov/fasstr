...
The following is an example of the first 6 metrics of the **Annual_Trends_Results** tibble:
...
results <- head(as.data.frame(trends[[2]]))
results
```

The following is an example of the outputed **Annual_Trends_Results** tibble from the Mission Creek HYDAT station from 1973 to 2013:
...
setting a zyp_alpha will plot a line if the significance value is equal or less than the alpha value.  TO plot a line through all of them, set eh value to 1. Example below witha alpha of 0.05. NA if nothing.
...
```{r, eval=FALSE}
fasstr::compute_annual_trends(station_number = "08NM116",
                      zyp_method = "yuepilon",
                      start_year = 1973, end_year = 2013)
```
