...
Some values with NA are provided with subtitute values for plotting so that the plot is still produced, but the data is not shown in the plot.  The user can then modify the function to get they data they need.

As many plots may be created, fasstr function utilize tidyverse packages (dplyr, tidyr, and purrr) to create a tibble of plots.  See this [link](http://www.brodrigues.co/blog/2017-03-29-make-ggplot2-purrr/) for more information on the methods. This method allows for the creation of many plots in an efficient manner.

- Group the data by STATION_NUMBER (or other grouping depending on the function) for each plot using dplyr::group().
- Then using tidyr::nest() a tibble is created with two columns, the first being the STATION_NUMBER and the second being a tibble of data for each STATION_NUMBER (a tibble of tibbles).
- Then using dplyr::mutate() a column of ggplots is created; and using purrr::map2() all of the plots can be created simultaneously (thus saving time in the function).
- As this type of table isn't entirely user-friendly, each plot is extracted into a list and named appropriately.

### Writing Functions (write_)
...
- **writexl** - used to write data as Excel files
