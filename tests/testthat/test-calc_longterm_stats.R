context("Calc longterm stats")

test_that("creates a dataframe with the proper columns", {
  skip_on_cran()
  skip_on_travis()
  data <- calc_longterm_stats(station_number = "08NM116", start_year = 1980)
...
test_that("outputs data for two stations", {
  skip_on_cran()
  skip_on_travis()
  data <- calc_longterm_stats(station_number = c("08NM116","08HB048"), start_year = 1980)
...
test_that("produces NA if there is missing data and warning is produced", {
  skip_on_cran()
  skip_on_travis()
  data <- suppressWarnings(calc_longterm_stats(station_number = "08NM116",
                              ignore_missing = FALSE))
  expect_true(any(is.na(data)))
  expect_warning(calc_longterm_stats(station_number = "08NM116",
                                     ignore_missing = FALSE))
})
...
test_that("ignore_missing calculates all data", {
  skip_on_cran()
  skip_on_travis()
  data <- calc_longterm_stats(station_number = "08NM116",
                              ignore_missing = TRUE)
  expect_true(any(!is.na(data)))
})
...
test_that("rolling days are applied properly", {
  ...
  data <- calc_longterm_stats(data = flow_data,
