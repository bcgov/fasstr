context("Plot flow duration")

test_that("a list of plots is created", {
  skip_on_cran()
  skip_on_travis()
  plots <- plot_flow_duration(station_number = "08NM116", start_year = 1980)
...
test_that("multiple plots are created with multiple groups", {
  skip_on_cran()
  skip_on_travis()
  plots <- plot_flow_duration(station_number = c("08NM116","08HB048"), start_year = 1980)
