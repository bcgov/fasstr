...
fasstr_add_cumulative_yield <- function(flowdata=NULL,
  ...
  flowdata.temp <- fasstr::fasstr_add_missing_dates(flowdata=flowdata)
  ...
  # Add cumulative
  flowdata.temp <- dplyr::mutate(dplyr::group_by(flowdata.temp,AnalysisYear),Cumul_Yield=cumsum(Value)*86400/(basin_area*1000))
  
  
  # Return flowdata to original dates
  flowdata.temp <- dplyr::select(dplyr::ungroup(flowdata.temp),Date,Cumul_Yield)
  
  flowdata <- merge(flowdata,flowdata.temp,by="Date",all.x = T)
  flowdata <-  flowdata[,c(col.ord,paste("Cumul_Yield"))]
