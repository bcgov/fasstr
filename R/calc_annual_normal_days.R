...
calc_annual_normal_days <- function(data,
                                    dates = Date,
                                    values = Value,
  ...
  normals_stats <- dplyr::summarise(dplyr::group_by(flow_data_temp, STATION_NUMBER, WaterYear),
                                    Normal_Days = sum(Value >= LOWER & Value <= UPPER, na.rm = FALSE),
                                    Below_Normal_Days = sum(Value < LOWER, na.rm = FALSE),
                                    Above_Normal_Days = sum(Value > UPPER, na.rm = FALSE))
