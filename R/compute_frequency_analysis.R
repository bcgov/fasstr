...
compute_frequency_analysis <- function(flowdata=NULL,
                                       HYDAT=NULL,
                                       HYDAT_peaks=NULL,
  ...
  if( is.null(HYDAT_peaks) ) {
    if( all(is.na(rolling_days)) )     {stop("rolling_days argument must be provided")}
  ...
  if ( is.null(HYDAT_peaks) ) {
    ...
    for (day in rolling_days) {
      ...
      flowdata_temp <- fasstr::add_rolling_means(flowdata_temp,days = day,align = rolling_align)
