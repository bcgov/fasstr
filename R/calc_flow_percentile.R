...
#' @param roll_days Numeric vector of the number of days to apply the rolling mean. Default \code{1}.
...
calc_flow_percentile <- function(data = NULL,
  ...
  if(!is.numeric(flow_value))  stop("A numeric flow_value is required.")
  if(length(flow_value)>1)     stop("Only one numeric flow_value is required.")
