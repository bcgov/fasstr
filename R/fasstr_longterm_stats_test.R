...
fasstr_longterm_stats_test <- function(flowdata=NULL,
                                  HYDAT=NULL,
                                  percentiles=c(10,90),
                                  water_year=FALSE,
                                  water_year_start=10,
  ...
  flowdata <- dplyr::select(flowdata,STATION_NUMBER,Date,Value)
  ...
  # something about end year before mindate or start data after start
  ...
  #levels(flowdata$MonthName) <- c(levels(flowdata$MonthName),"Long-term")
  # Calculate the monthly and longterm stats
  Q_month_longterm <-   dplyr::summarize(dplyr::group_by(flowdata,STATION_NUMBER,MonthName),
                                         Mean = mean(Value,na.rm=TRUE),
                                         Median = median(Value,na.rm=TRUE),
                                         Maximum = max(Value,na.rm=TRUE),
                                         Minimum = min(Value,na.rm=TRUE))
  Q_month_longterm <- dplyr::ungroup(Q_month_longterm)
  Q_all_longterm <-   dplyr::summarize(dplyr::group_by(flowdata,STATION_NUMBER),
                                       Mean = mean(Value,na.rm=TRUE),
                                       Median = median(Value,na.rm=TRUE),
                                       Maximum = max(Value,na.rm=TRUE),
                                       Minimum = min(Value,na.rm=TRUE))
  Q_all_longterm <- dplyr::mutate(Q_all_longterm,MonthName=as.factor("Long-term"))
  ...
  if (!all(is.na(percentiles))){
    for (ptile in percentiles) {
      ...
      Q_month_longterm_ptile <- dplyr::summarise(dplyr::group_by(flowdata,STATION_NUMBER,MonthName),
                                                 Percentile=quantile(Value,ptile/100, na.rm=TRUE))
      Q_month_longterm_ptile <- dplyr::ungroup(Q_month_longterm_ptile)
      Q_all_longterm_ptile <- dplyr::summarise(dplyr::group_by(flowdata,STATION_NUMBER),
                                               Percentile=quantile(Value,ptile/100, na.rm=TRUE))
      ...
      colnames(Q_longterm_ptiles)[3] <- paste0("P",ptile)
      
      Q_longterm <- merge(Q_longterm,Q_longterm_ptiles,by=c("STATION_NUMBER","MonthName"))
    }
  }
  ...
  if( is.numeric(custom_months) & all(custom_months %in% c(1:12)) ) {
    ...
    Q_month_custom <-   dplyr::summarize(dplyr::group_by(flowdata_temp,STATION_NUMBER),
                                         Mean = mean(Value,na.rm=TRUE),
                                         Median = median(Value,na.rm=TRUE),
                                         Maximum = max(Value,na.rm=TRUE),
                                         Minimum = min(Value,na.rm=TRUE))
    Q_month_custom <- dplyr::mutate(Q_month_custom,MonthName=paste0(custom_months_label))
    Q_month_custom <- dplyr::ungroup(Q_month_custom)
    
    
    if (!all(is.na(percentiles))){
      for (ptile in percentiles) {
        Q_ptile_custom <- dplyr::summarise(dplyr::group_by(flowdata_temp,STATION_NUMBER),
                                           Percentile=quantile(Value,ptile/100, na.rm=TRUE))
        Q_ptile_custom <- dplyr::mutate(Q_ptile_custom,MonthName=paste0(custom_months_label))
        
        colnames(Q_ptile_custom)[2] <- paste0("P",ptile)
        Q_month_custom <- merge(Q_month_custom,Q_ptile_custom,by=c("STATION_NUMBER","MonthName"))
      }
    }
  ...
  if (transpose) {
    col_names <- names(Q_longterm[-(1:2)])
  ...
  if( all(Q_longterm$STATION_NUMBER=="00AA000") ) {
    Q_longterm <- Q_longterm[,-1]
    if (write_table) {
      temp <- temp[,-1]
    }
  }
  ...
  if (write_table) {
    file_stat_csv <-file.path(write_dir,"longterm-summary-statistics.csv")
