...
#'
#' @description Plots the annual data and significant trend from the results from all statistics from a compute_annual_trends()
#'    analysis.
#' 
#' @param trends_results Data frame of results from the compute_annual_trends() function, with original data included
#'    (incl_data = TRUE). The first column must be 'Statistics' with the following columns with results and then data.
#' @param zyp_alpha Numeric value of the significance level of when to plot a trend line. Set to NA if no line required.
#' 
...
#' \dontrun{
#' 
#' trends <- compute_annual_trends("08NM116",
#'                                 zyp_method = "yuepilon",
...
plot_annual_trends <- function(trends_results = NULL,
  ...
  if(is.null(trends_results))        stop("Must provide a data frame of compute_annual_trends() results with incl_data = TRUE.")
  if(!is.data.frame(trends_results)) stop("Must provide a data frame of compute_annual_trends() results with incl_data = TRUE.")
  
  if(!all(c("lbound", "trend", "trendp", "tau", "sig", "linear", "n_years") %in% names(trends_results)))
    stop("Must provide a data frame of compute_annual_trends() results with incl_data = TRUE.")
